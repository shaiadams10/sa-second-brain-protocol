...
def report(paths: RuntimePaths, *, include_memory: bool = True) -> dict[str, Any]:
    ...
    data: dict[str, Any] = {
        ...
        "codex": codex_version,
        "basic_memory_package": _version([str(Path(shutil.which("basic-memory") or "basic-memory")), "--version"]),
        "graphify_package": _version([str(Path(shutil.which("graphify") or "graphify")), "--version"]),
        "git": _version(["git", "--version"]),
        "github_cli": _version(["gh", "--version"]),
        "schedule": task_status(config["task_name"]),
