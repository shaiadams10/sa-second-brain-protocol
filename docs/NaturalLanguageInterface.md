...
11. This immediate write route exists only inside the main vault. While MCP transport is disabled, agents in other projects cannot submit candidates; their sessions remain read-only evidence and enter through Daily ingestion. A future opted-in MCP route may submit a bounded candidate into Curate, but it can never write canonical notes directly or promote a claim merely because the caller labels it certain.
