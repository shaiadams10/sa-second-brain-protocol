...
- Before automatic Git work, a stale `index.lock` may be cleared only when it is zero-byte, at least one hour old, and no Git process is running. Fresh, non-empty, active, or ambiguous locks remain blocking so automation fails closed instead of risking repository corruption.
