...
- While MCP transport is disabled, other project agents remain read-only evidence sources and cannot submit Curate candidates; their session evidence enters through the normal Daily pipeline. A future authenticated MCP may accept size-bounded candidate submissions from an explicitly opted-in project, but never direct canonical-note writes. Caller confidence cannot bypass host validation: only the private Brain's deterministic promotion policy may confirm an objective, non-sensitive, non-conflicting claim; everything else remains reviewable in Curate.
...
The private repository is the backup for the canonical vault and may contain personal knowledge. The public repository is a one-way sanitized distribution of the reusable protocol only. It must never receive personal vault folders, populated dashboard output, runtime state, or evidence; a public repository is never a source of private knowledge and never a target for MCP access.
